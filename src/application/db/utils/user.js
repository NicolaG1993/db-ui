...
module.exports.setUserNewVerificationToken = (
    client,
    newToken,
    now,
    userId
) => {
    const myQuery = `UPDATE users SET verification_token = $1, last_verification_email = $2 WHERE id = $3`;
    const keys = [newToken, now, userId];
    return client.query(myQuery, keys);
};
